...
def _did_upload_today_run_dates():
    ...
    dt_str = str(korean_financial_minute_data_miner.util.time.get_utcnow().astimezone(tz).date())
...
def on_upload():
    global _run_dates
    _run_dates.add(korean_financial_minute_data_miner.util.time.get_today_str_tz())
