import os
import pandas as pd
import korean_financial_minute_data_miner.util.time, korean_financial_minute_data_miner.util.dir
...
def _combine_all_in_filenames(filenames):
    ...
    for filename in filenames:
        ...
        df['datetime'] = pd.to_datetime(df['datetime'], format=korean_financial_minute_data_miner.util.time.DATETIME_FORMAT)
...
def combine_for_date(date_str, data_type):
    ...
    '''
    latest = korean_financial_minute_data_miner.util.time.get_latest_time_v(date_str, data_type)
    if latest == 0:
        return pd.DataFrame()
